...
// 绘制挤出的材质
async function drawExtrudeMesh(
  point: [number, number][],
  projectionFn: any
) {
  ...
  //     console.error(`Found NaN value at index ${i}`);
  //     return;
  //   }
  // }
  // 加载纹理
  const texture: any = await loadTexture('/images/home-bg-4.png');

  const material = new THREE.MeshBasicMaterial({
    // map: texture,
    color: mapConfig.mapColorGradient[Math.floor(Math.random() * 4)], // 随机颜色
    wireframe: false, // 你可能想要关闭wirframe设置，更自然的地图材质
    side: THREE.DoubleSide, // 确保材质的双面渲染
...
// 生成地图3D模型
export async function generateMapObject3D(
  mapdata: GeoJsonType,
  ...
  // 每个省的数据
  for (const basicFeatureItem of basicFeatures) {
    // 每个省份的地图对象
    ...
    if (!basicFeatureItem.properties.centroid) {
      basicFeatureItem.properties.centroid = basicFeatureItem.properties.center
    }
    // 每个中心点位置
    ...
    if (featureType === "MultiPolygon") {
      for (const multiPolygon of featureCoords) {
        for (const polygon of multiPolygon) {
          const { mesh, line } = await drawExtrudeMesh(polygon, projectionFn);
    ...
    if (featureType === "Polygon") {
      for (const polygon of featureCoords) {
        const { mesh, line } = await drawExtrudeMesh(polygon, projectionFn);
        provinceMapObject3D.add(mesh);
        provinceMapObject3D.add(line);
      };
...
// 线上移动物体
export const drawflySpot = async (curve: any) => {
  const aGeo = new THREE.SphereGeometry(0.2);
  // 加载纹理
  const texture: any = await loadTexture('/images/plane.jpg');
...
// 绘制两点链接飞线
export const drawLineBetween2Spot = async (
  ...
  const flySpot = await drawflySpot(curve);
