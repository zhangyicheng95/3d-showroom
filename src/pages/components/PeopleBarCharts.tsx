...
const PeopleBarCharts: React.FC<Props> = (props: any) => {
  let { data = [] } = props;
  ...
  useEffect(() => {
    setTimeout(() => {
      ...
      const yData = data?.map((i: any) => i.value).sort((a: any, b: any) => a - b);
      const option = {
        ...options,
        grid: {
          ...options.grid,
        ...
        series: [
          {
            ...
            symbolOffset: [20, 0],
            symbolBoundingData: Math.max(...yData) * 10,
            data: yData,
            z: 10,
          },
          {
...
export default PeopleBarCharts;
